...
- (id)initWithName:(CPString)aName delegate:(id)anObject
{
    ...
    if (self)
    {
        ...
        _db.transaction(function(_db){
                _db.executeSql( 'CREATE TABLE IF NOT EXISTS InventoryFileCache (key TEXT UNIQUE NOT NULL PRIMARY KEY, value TEXT NOT NULL)' );
        });
    }
    return self;
}
...
- (void)setValue:(CPString)aValue forKey:(CPString)aKey
{
    ...
    _db.transaction(function(db){
        ...
        db.executeSql("UPDATE InventoryFileCache SET value = ? WHERE key = ?",[aValue, aKey], function result(tx, rs){}, function anError(tx, err){});
        //if the key exists this will fail quietyl, if not it will add the key to the db.
        db.executeSql("INSERT INTO InventoryFileCache (key, value) VALUES (?, ?)", [aKey, aValue], function result(db, rs){}, function error(db, err){});
    });
...
{
    _db.transaction(function(db){
        db.executeSql("SELECT value FROM InventoryFileCache WHERE key=?",[aKey], function result(text, result){[self _parseResults:result]; [[CPRunLoop currentRunLoop] limitDateForMode:nil]}, function anError(text, theError){});
    });
}
...
{
    _db.transaction(function(db){
        db.executeSql("DELETE FROM InventoryFileCache WHERE key=?",[aKey], function result(text, result){}, function anError(text, theError){});
    });
